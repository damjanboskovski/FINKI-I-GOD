...
public:
    ...
    double getBuy_Price(){ return buy_price; } int getYear(){ return manufacturing_year; }
...
class Warehouse{
private:
    char location[60], name[50]; int opening_year, num; Guitar *arr;
public:
    ...
    Warehouse(const Warehouse &w){
        ...
        for(int i =0;i<w.num;i++){ arr[i] = w.arr[i]; } num = w.num;
    }
