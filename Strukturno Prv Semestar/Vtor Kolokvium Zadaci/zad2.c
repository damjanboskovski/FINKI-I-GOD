...
void wtf() {
    ...
    while((c = getchar()) != EOF) { fputc(c, f); } fclose(f);
}

int main()
{
    wtf(); FILE *read = fopen("broevi.txt","r"); int max = 0, flag = 0; char string[size];

    while(fgets(string,size, read) != NULL){
        char temp[size], number[size]; strcpy(temp, string); char *num = strtok(temp, " ");

        while(num != NULL){
            if((int)(*(num) - '0') > max && flag ){ max = (int)(*(num) - '0'); strcpy(number,num); number[strcspn(number,"\n")] = 0; }
